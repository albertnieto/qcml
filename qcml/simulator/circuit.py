...
class QuantumCircuit:
    ...
    def rx(self, target: int, theta):
        self.gates.append({'name': 'RX', 'targets': [target], 'params': {'theta': theta}})
    
    def ry(self, target: int, theta):
        self.gates.append({'name': 'RY', 'targets': [target], 'params': {'theta': theta}})
    
    def rz(self, target: int, theta):
        self.gates.append({'name': 'RZ', 'targets': [target], 'params': {'theta': theta}})
    
    def cnot(self, control: int, target: int):
        self.gates.append({'name': 'CNOT', 'targets': [control, target]})
    
    def add_gate(self, name: str, targets: List[int], params: Optional[Dict[str, Any]] = None):
