...
from qic.utils.hyperparam_search_utils import read_data, construct_hyperparameter_grid, csv_to_dict
...
class ModelScoring:
    def __init__(self, classifier_name, results_path="", experiment_name='default_exp',
                 hyperparameter_scoring=None, hyperparameter_refit="accuracy",
                 plot_loss=True, n_jobs=-1, *args, **kwargs):
    ...
    def find_optimal_hyperparameters(self, X, y, experiment_name=None):
        ...
        logging.info(" ".join([classifier_name, "Exp:", self.experiment_name, "Train acc:", str(acc_train), "Time single run", str(b - a)]))
        ...
        gs = GridSearchCV(estimator=classifier, param_grid=self.hyperparam_grid,
        ...
        self.best_hp_path = os.path.join(results_path, results_filename_stem + '-best-hyperparameters.csv')
    ...
    def score(self, X_train, X_test, y_train, y_test, experiment_name=None):
        ...
        self.best_hp_path = os.path.join(self.results_path, results_filename_stem + '-best-hyperparameters.csv')
        if self.best_hp_path:
            path_out = os.path.join(self.results_path, results_filename_stem + "-best-hyperparams-results.csv")
    ...
    def _log_settings(self):
        ...
        logging.info("Hyperparam grid: " + " ".join(
            [f"{key}: {self.hyperparam_grid[key]}" for key in self.hyperparam_grid.keys()]
        ))
