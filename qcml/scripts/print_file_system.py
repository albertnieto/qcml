...
try:
    ...
    def colored(text, color=None):
        return text  # Just return the text without any color if termcolor is not available
...
def build_tree(directory, only_dirs=False, include_functions=False, show_hidden=False, show_pycache=False):
    """
    ...
    for entry in entries:
        ...
        if os.path.isdir(path):
            tree[entry] = build_tree(path, only_dirs, include_functions, show_hidden, show_pycache)
...
def print_tree(tree, prefix="", show_types=False, colored_output=True):
    ...
    for i, entry in enumerate(entries):
        ...
        entry_type = tree[entry].get('_type', None) if isinstance(tree[entry], dict) else None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json", action="store_true", help="Return the directory tree in JSON format.")
    parser.add_argument("--include-functions", action="store_true", help="Include classes and functions of each Python file in the tree.")
    parser.add_argument("--show-types", action="store_true", help="Show D (directory), M (module), C (class), F (function) indicators.")
    parser.add_argument("--colored", action="store_true", default=True, help="Colored output for better aesthetics (default: True).")
    parser.add_argument("--show-hidden", action="store_true", help="Show hidden directories (those starting with a dot).")
    parser.add_argument("--show-pycache", action="store_true", help="Show __pycache__ directories (hidden by default).")
    
    args = parser.parse_args()

    tree = build_tree(args.directory, args.only_dirs, args.include_functions, args.show_hidden, args.show_pycache)
