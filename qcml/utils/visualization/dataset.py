...
def plot_2d_dataset(
    ...
    if features.shape[1] > 2:
        ...
        df_combined["Class"] = pd.concat(
            [df_train.iloc[:, -1], df_test.iloc[:, -1]], ignore_index=True
        )
    else:
    ...
    if decision_boundary is not None:
        ...
        plot_decision_boundary(
            X, y, decision_boundary, ax, palette, saturation_factor=saturation_factor
        )

    # Plot the data points
    sns.scatterplot(x=X[:, 0], y=X[:, 1], hue=y, palette=palette, ax=ax, edgecolor="k")
